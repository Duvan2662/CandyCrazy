...
import com.example.candy.databinding.ActivityDosBinding
...
class Dos : AppCompatActivity() {
    ...
    //Funcion Tiempo
    private fun Tiempo(){
        val intent = Intent(this, Dos::class.java)//Actividad Uno
        val intent2 = Intent(this, Perdedor::class.java)//Actividad Inicial

        countDownTimer = object : CountDownTimer(120000, 1000) {//Defino el tiempo
        override fun onTick(millisUntilFinished: Long) {
            ...
            override fun onFinish() {
                ...
                val builder = AlertDialog.Builder(this@Dos)//Se muestra en la actividad 1
                ...
                boton.setOnClickListener {
                    if (posicionActual == null) {
                        ...
                        if ((i == filaActual - 1 && j == columnaActual) ||
                            ...
                            val botonActual =
                                tablero.getChildAt(filaActual * 8 + columnaActual) as Button
    ...
    private fun propiedades(boton: Button, i: Int, j: Int, tablero: GridLayout) {
        ...
        boton.layoutParams = TableRow.LayoutParams(120, 120)
    ...
    private fun pintar(boton: Button, i: Int, j: Int, tablero: GridLayout) {
        val colores = listOf("#BB8FCE", "#7FB3D5", "#F8C471", "#7DCEA0", "#F160C5", "#DAF7A6")
        ...
        } while (i > 0 && (tablero.getChildAt((i - 1) * 8 + j).background as? ColorDrawable)?.color == color ||
            j > 0 && (tablero.getChildAt(i * 8 + j - 1).background as? ColorDrawable)?.color == color
        )
                ...
                if (contador == 3) {
                    ...
                    for (k in j - 2..j) {
                        if (k > 0) {
                            ...
                            val color = (boton.background as? ColorDrawable)?.color
                            val colorSiguiente = (tablero.getChildAt(i * columnas + k + 1).background as? ColorDrawable)?.color
                            if (color == colorAnterior && color == colorSiguiente) {
                                pintar(tablero.getChildAt(i * columnas + k - 1) as Button, i, k - 1, tablero)
                                pintar(tablero.getChildAt(i * columnas + k + 1) as Button, i, k + 1, tablero)
                            }
                        }
                    }
    ...
    fun Reiniciar(){
        ...
        val intent = Intent(this, Dos::class.java)//Activity info1
